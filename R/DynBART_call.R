...
#'
#'Bayesian Additive Regression Trees Modeling for Continuous Outcome for Dynamic Data,
#'@param formula response ~ covariates,
#'@param data Training data with the continuous response,
...
#'x1 = x1[rep(1:n,nd),]
#'x2 = runif(n*nd) # simulated dynamic variable
#'x = cbind(x1, x2)
#'Ey = f(x)
#'y=Ey+sigma*rnorm(n*nd)
#'tdat = data.frame(x,y)
#'###############################################
...
#'bfit = DynBART_call(fml, data = dat, test.data = tdat,
...
#'@export
#'@useDynLib allBART
DynBART_call  <- function(formula, data, test.data = NULL,
                       Prior = NULL, Mcmc = NULL, diagnostics = TRUE)
{
  ...
  if (!is.null(test.data)){
    ...
    stop(paste("Error: no test data, please use BART_call instead."))
  }
  ...
  if (!is.null(test.data)){
    ...
    stmp = rep(sigmasample,each = testnsub)
    vec_samp_test = rnorm(testn, yhat.test, stmp)
